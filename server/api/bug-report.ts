...
const sanitizeSeverity = (severity: any) => {
...
const isAdminSubmission = (payload: any, event: any, env: any) => {
...
const formatDiagnostics = (diagnostics: any) => {
  if (!diagnostics) return "{}";
  if (typeof diagnostics === "string") return diagnostics;
  return JSON.stringify(diagnostics, null, 2);
};

async function createPaperclipIssue(payload: any, env: any) {
...
async function storeHumanAdminReport(payload: any, env: any) {
  ...
  // On Cloudflare Pages, STITCH_BUG_REPORTS is bound as a KV namespace
